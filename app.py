...
# ── 요일별 추천 식단 (Python 월=0, 일=6) ──
MEALS = {
    0: dict(m="달걀 2개+그릭요거트+사과+당근스틱+호두",   l="추어탕+잡곡밥+깍두기",             d="달걀 스크램블+그릭요거트+블루베리+방울토마토"),
    1: dict(m="달걀 2개+그릭요거트+배+방울토마토+아몬드", l="추어탕+잡곡밥+깍두기",             d="삶은달걀 2개+그릭요거트+수박+당근스틱"),
    2: dict(m="달걀 2개+그릭요거트+사과+브로콜리+호두",   l="닭갈비+현미밥+된장국(무두부)",      d="달걀 스크램블+그릭요거트+블루베리+브로콜리"),
    3: dict(m="달걀 2개+그릭요거트+배+파프리카+아몬드",   l="설렁탕+잡곡밥+깍두기",             d="삶은달걀 2개+그릭요거트+사과+방울토마토"),
    4: dict(m="달걀 2개+그릭요거트+수박+당근스틱+호두",   l="소고기 국밥+잡곡밥+무생채",        d="달걀 스크램블+그릭요거트+배+당근스틱"),
    5: dict(m="달걀 2개+그릭요거트+사과+브로콜리+아몬드", l="삼계탕(주말보양)+잡곡밥",          d="삶은달걀 2개+그릭요거트+수박+브로콜리"),
    6: dict(m="달걀 2개+그릭요거트+배+방울토마토+호두",   l="소고기 된장찌개+현미밥+연근조림",  d="달걀 스크램블+그릭요거트+사과+파프리카"),
}
...
def save_idea(text, tag):
    try:
        ws = get_worksheet("메모아이디어", ["날짜","시간","태그","내용"])
...
if st.session_state.shopping_items is None:
    st.session_state.shopping_items = get_base_shopping()
...
<style>
  ...
  .stTextInput>div>div>input, .stTextArea>div>div>textarea { border-radius: 8px; font-size: 14px; }
...
with tab1:
    ...
    # 수분 (ml, 250ml 단위, 목표 2000ml)
    st.markdown("<div class='card-title'>💧 수분 섭취 (목표 2,000ml)</div>", unsafe_allow_html=True)
    new_water = st.slider("수분(ml)", 0, 2000, st.session_state.water, step=250, label_visibility="collapsed")
    ...
    # Google Sheets 버튼
    st.markdown(
        f'<a href="{SHEETS_URL}" target="_blank"><button style="width:100%;padding:10px;margin-top:8px;background:#1D9E75;color:white;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">📊 Google Sheets 열기</button></a>',
        unsafe_allow_html=True
    )
...
with tab3:
    ...
    for i, todo in enumerate(st.session_state.todos):
        col1, col2 = st.columns([5,1])
        with col1:
            tc   = TAG_COLORS.get(todo["tag"],"#F0F0F0")
            styl = "text-decoration:line-through;color:#9E9E9A;" if todo["done"] else ""
            st.markdown(f"<span style='{styl}'>{todo['text']}</span> &nbsp; <span style='background:{tc};font-size:10px;padding:2px 7px;border-radius:10px;'>{todo['tag']}</span>", unsafe_allow_html=True)
        with col2:
...
with tab5:
    ...
    diary = st.text_area("일기", value=st.session_state.diary_content, height=200,
...
with tab6:
    ...
    for category, items in st.session_state.shopping_items.items():
        ...
        for i, item in enumerate(items):
            ...
            with col_c:
                checked = st.checkbox("체크", value=item["check"],
                                      key=f"shop_{category}_{i}", label_visibility="collapsed")
                if checked != item["check"]:
                    st.session_state.shopping_items[category][i]["check"] = checked
            with col_n:
                s = "text-decoration:line-through;color:#9E9E9A;" if item["check"] else ""
                st.markdown(f"<span style='{s}'>{item['name']}</span>", unsafe_allow_html=True)
            with col_q:
                st.caption(item["qty"])
            if checked:
                checked_total += 1
    ...
    if st.session_state.custom_items:
        ...
        for i, item in enumerate(st.session_state.custom_items):
            ...
            with col_c:
                checked = st.checkbox("체크", value=item["check"],
                                      key=f"custom_{i}", label_visibility="collapsed")
            ...
            with col_n:
                ...
                name = item["name"]
                qty  = item["qty"]
                st.markdown(f"<span style='{s}'>{name} <span style='color:#9E9E9A;font-size:12px;'>{qty}</span></span>", unsafe_allow_html=True)
            with col_d:
    ...
    st.markdown(
        f'<a href="{SHEETS_URL}" target="_blank"><button style="width:100%;padding:10px;margin-top:12px;background:#1D9E75;color:white;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">📊 Google Sheets 열기</button></a>',
        unsafe_allow_html=True
    )
